# CliPeek

> 所有 AI agent,角落里一眼看全。

[![CI](https://github.com/Jirry/clipeek/actions/workflows/ci.yml/badge.svg)](https://github.com/Jirry/clipeek/actions/workflows/ci.yml)

中文 · [English](README.en.md)

**CliPeek**(瞄一眼)是一个常驻屏幕角落、置顶悬浮的 macOS HUD,实时监控你本地的 AI CLI 会话,把每个会话显示成一颗红绿灯——不用在一堆终端标签页里翻找,一眼就知道哪个 agent 在干活、哪个在等你、哪个已经完事,点一下或按快捷键就能跳到它的终端。

<!-- TODO: 这里放一张 HUD 的截图 / GIF -->

## 灯语

| 灯 | 含义 |
| --- | --- |
| 🔴 红·常亮 | 异常——API 报错 / 崩溃 |
| 🔴 红·闪烁 | 需介入——权限弹窗 / 提问 / 计划审批 |
| 🟡 黄·常亮 | 执行中——思考 / 跑命令 / 流式回复 |
| 🟢 绿·闪烁 | 该你了——刚结束,或 agent 在等你输入 |
| ⚪ 灭·暗点 | 完成——空闲搁置(占位不抢眼) |

按紧急程度排序(红 ▸ 红闪 ▸ 黄 ▸ 绿闪 ▸ 灭),同状态按最近活动排前。已关闭的会话会自动消失。

> 以上是**默认灯语**。每种状态对应什么灯效——颜色(红 / 黄 / 绿)、是否闪烁、闪烁频率,或干脆设成「灭」——都可在**设置 → 灯语**里自定义。

## 快捷键

| 快捷键 | 效果 |
| --- | --- |
| **`⌘J`**(Command + J) | **智能跳转**:优先跳到最需要你关注的会话 |
| **`⌘⇧J`**(Command + Shift + J) | **全量跳转**:无视状态,在所有灯之间按顺序循环 |
| **单击灯 / 列表行** | 跳到该会话所在的终端标签页 |

两个快捷键都会**聚焦目标会话的终端**,并把那盏灯**白环 + 脉冲高亮约 2.5 秒**(连按跳到下一个时,上一个立即复原);两者共用同一个「当前选中」,「下一个」总是从当前那盏往后走。

- **`⌘J`(智能)**:先在「活跃」会话里循环,按紧急度 **🔴 异常 ▸ 🔴 红闪(需介入)▸ 🟡 执行中 ▸ 🟢 绿闪(该你了)**;这些活跃会话一个都没有时,才退而在其余(已完成 / 空闲)会话里循环。
- **`⌘⇧J`(全量)**:无视状态,在**所有灯**之间按灯条顺序循环——始终能切到任意一盏(含执行中、绿灯)。

> 两个快捷键都可在**设置**里自定义(录制你想要的组合)。注意 `⌘J` 是全局快捷键,会盖过其它应用里的 ⌘J。

## 功能

- **每个存活会话一颗灯**,排成一条;灯下可显示会话名(可开关)。
- **同时盯多个 CLI** —— Claude Code 与 Codex 会话并排显示;用**灯的形状**区分是哪个 CLI(颜色仍表状态),形状可在设置里改。
- **悬停**某颗灯 → 弹出该会话的名字和工作目录;点 ✏︎ 可改名(仅本地显示名,不动你的真实会话)。
- **两种布局**,一键切换:
  - **横排** —— 一条横向灯带,贴着 Dock。
  - **竖排** —— 屏幕角落的竖向列表,带一行摘要、名字、路径和闲置时长。
- **可移动 / 滚动 / 缩放 / 调大小** —— 拖手柄移动(可跨多显示器,也能贴到屏幕顶部菜单栏旁);在灯上拖动可横向滚动;拖边缘调大小(压到菜单栏高度时,灯也跟着缩小);缩放在设置里调。位置、大小、改名都跨重启记住。
- **位置预设 / 快照** —— 三个一键预设(右下角、左下角、顶部贴菜单栏);也能把当前的位置 / 大小 / 缩放 / 布局存成命名快照(最多 3 个),随时一键还原。
- **自定义灯语** —— 每种状态(异常 / 需介入 / 执行中 / 该你了 / 完成)用什么灯效都可改:颜色(红 / 黄 / 绿)、是否闪烁、闪烁频率,或设为「灭」(只盯你在意的状态);设置里实时预览、一键恢复默认。
- **自动更新** —— 自动检查新版本、后台下载,一键重启即完成更新(仅打包版;从源码运行不涉及)。
- **菜单栏托盘** —— 无 Dock 图标、无窗口边框。菜单栏图标可选**固定图标**(单色三盏灯,随明暗自动反色)或**红绿灯**(按最紧要的那盏灯着色,该状态闪烁则跟着闪);可选显示存活会话数。**左键单击**跳到最该关注的会话、**右键**弹菜单。

## 设置

菜单栏图标(或在灯上右键)→ **设置…**:

- **快捷键** —— 自定义 `⌘J`(智能跳转)和 `⌘⇧J`(全量跳转),录制按键即可。
- **外观** —— 缩放、横 / 竖排、灯下是否显名。
- **灯语** —— 自定义每种状态的灯效(颜色 / 闪烁 / 频率 / 灭),逐行实时预览,可一键恢复默认。
- **图形** —— 为每个 CLI(Claude / Codex)指定灯的形状(圆 / 方 / 三角 / 菱形 / 五边形 / 六边形 / 星形 / 星芒);形状分 CLI、颜色分状态。
- **窗口** —— **预设位置**(右下角 / 左下角 / 顶部贴菜单栏)一键就位;**自定义位置**把当前的位置、大小、缩放、横 / 竖排存成快照(最多 3 个、可重命名),点一下即还原。
- **通用** —— 开机自启(登录系统时自动启动 CliPeek);**菜单栏图标**(固定图标 / 红绿灯)与**是否显示数字**(切样式会自动调整数字默认);**软件更新**(检查 / 后台下载 / 一键重启更新)。

## 环境要求

- macOS
- [Node.js](https://nodejs.org/)(从源码运行)
- [Claude Code](https://claude.com/claude-code)
- [Codex CLI](https://github.com/openai/codex)(可选;想监控 Codex 会话时)
- [Warp](https://www.warp.dev/)——跳转终端标签页需要(需 Warp ≥ `2026.05.27`)。其它终端照样显示状态,只是暂时跳不过去。

## 从源码运行

```bash
git clone https://github.com/Jirry/clipeek.git
cd clipeek
npm install
npm start        # 构建 + 启动
# npm run dev    # 开发时监听重建
```

## 打包发版

```bash
npm run dist     # electron-builder → release/CliPeek-<版本>-{arm64,x64}.{dmg,zip}
```

产出**未签名**(无 Apple 开发者证书)。macOS 对未签名应用首次打开会被 Gatekeeper 拦,在终端执行一次去掉隔离标记即可:

```sh
xattr -dr com.apple.quarantine /Applications/CliPeek.app
```

## 说明

首次启动时,CliPeek 会装一个小巧、可随时删除的 Claude Code hook(文件在 `~/.clipeek/`,注册进 `~/.claude/settings.json`),用来探测权限弹窗、一轮的开始与结束(及时切换灯色)、以及打开对应终端标签页。它只加自己那几条,绝不动你的其它 hook——删掉即可卸载。

## 进度

- **Claude Code** —— 已支持。
- **Codex** —— 已支持(交互式 CLI 会话;`codex exec` / 桌面版不纳入)。

## 许可

[MIT](LICENSE) © Jirry
